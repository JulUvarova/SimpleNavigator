...
AntColonyOptimizer::AntColonyOptimizer(s21_graph& graph, const AcoParams& params)
    : graph_(graph),
      params_(params),
...
void AntColonyOptimizer::Initialize() {
    pheromones_.assign(num_cities_, std::vector<double>(num_cities_, params_.initial_pheromone));
                ...
                heuristic_info_[i][j] = 0.0;
                 ...
                 if (dist > 0 && !std::isinf(dist)) {
                    heuristic_info_[i][j] = 1.0 / dist;
                     ...
                     heuristic_info_[i][j] = 1e-9; // Avoid division by zero, low attractiveness
                     pheromones_[i][j] = 0.0; // Optionally zero pheromone if no direct path
                 }
            }
        }
    }
    ...
    for (int iter = 0; iter < params_.num_iterations; ++iter) {
        ...
        for (int k = 0; k < params_.num_ants; ++k) {
            ...
            if (ant_tour_lengths[k] < best_result.distance) {
                ...
                // std::cout << "Iter " << iter << ", Ant " << k << ": New best tour length: " << best_result.distance << std::endl;
            }
        }
...
double AntColonyOptimizer::CalculateTourLength(const std::vector<int>& tour) const {
    double length = 0.0;
    if (tour.size() < 2) return std::numeric_limits<double>::infinity(); // Not a valid tour
...
int AntColonyOptimizer::SelectNextCity(int current_city, const std::vector<bool>& visited) {
    ...
    for (int next_city = 0; next_city < num_cities_; ++next_city) {
        if (!visited[next_city]) {
            ...
            // Consider only reachable cities with some attractiveness
             if (pheromone_level > 1e-9 || heuristic_level > 1e-9) { // Use a small threshold
    ...
    const std::vector<double>& ant_tour_lengths)
{
    // 1. Evaporation
    for (int i = 0; i < num_cities_; ++i) {
        for (int j = i + 1; j < num_cities_; ++j) { // Iterate only upper triangle for symmetric TSP
    ...
    for (size_t k = 0; k < ant_tours.size(); ++k) {
        ...
        if (tour_length <= 0 || std::isinf(tour_length) || std::isnan(tour_length)) {
             //std::cerr << "Warning: Skipping ant with invalid tour length: " << tour_length << std::endl;
