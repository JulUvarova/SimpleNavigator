 ...
 */
struct TsmResult {
  std::vector<int> vertices;  ///< An array with the route (vertex traverse order).
  double distance = 0;        ///< The length of this route.
};

/**
 * @brief Enumerates the algorithms available for solving the Traveling Salesman Problem.
 */
...
class s21_graph_algorithms {
 public:
   ...
   * @return A pair containing the shortest distance and the path (vector of vertices).
   *         If no path exists, distance is infinity and path is empty.
  ...
  /**
   * @brief Finds the shortest paths between all pairs of vertices using Floyd-Warshall algorithm.
   * @param graph The graph to process.
   * @return A matrix where element (i, j) is the shortest distance from vertex i to vertex j.
   *         std::numeric_limits<int>::max() indicates no path.
   */
   ...
   * @return A pair containing the total weight of the MST and the adjacency matrix of the MST.
   *         Returns {-1, {}} if the graph is not suitable (e.g., disconnected and unable to form a single tree, or not weighted undirected).
   */
   ...
   * @return A TsmResult struct containing the best route found and its distance.
   * @throw std::invalid_argument if the graph is not suitable for TSP (e.g., not complete, or has 0 vertices).
   */
