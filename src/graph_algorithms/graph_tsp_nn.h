...
/**
 * @brief Solves the Traveling Salesman Problem using the Nearest Neighbor
 * heuristic.
 *
...
class NearestNeighborSolver {
 public:
   ...
   *
   * The tour starts at vertex 0, visits each other vertex by always moving to
   * the nearest unvisited neighbor, and finally returns to vertex 0.
   *
   * @return A TsmResult struct containing the constructed tour and its total
   * distance. If the graph has 0 vertices, an empty tour with distance 0 is
   * returned. If the graph has 1 vertex, a tour {0, 0} with distance 0 is
   * returned. If at any point no unvisited reachable neighbor is found (for
   * disconnected graphs), the tour might be incomplete and its validity depends
 ...
 private:
   ...
   * visited.
   * @return The index of the nearest unvisited neighbor. Returns -1 if no
   * unvisited neighbor is found or if all reachable neighbors have been
   * visited.
   */
